...
import fr.vmz.jhipster.petclinic.service.PetQueryService;
import fr.vmz.jhipster.petclinic.service.PetService;
...
/**
 * REST controller for managing {@link fr.vmz.jhipster.petclinic.domain.Pet}.
 */
@RestController
...
public class PetResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new petDTO, or with status {@code 400 (Bad Request)} if the pet already has an ID.
    ...
    public ResponseEntity<PetDTO> partialUpdatePet(
        ...
        LOG.debug("REST request to partially update Pet : {}, {}", id, petDTO);
        if (petDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, petDTO.getId())) {
