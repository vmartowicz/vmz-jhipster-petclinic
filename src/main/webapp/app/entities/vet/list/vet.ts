...
import { VetDeleteDialog } from '../delete/vet-delete-dialog';
...
export class Vet {
  ...
  readonly isLoading = this.vetService.vetsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.vetService.vetsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.vets.set(this.fillComponentAttributesFromResponseBody([...this.vetService.vets()]));
    });
    effect(() => {
  ...
  delete(vet: IVet): void {
    const modalRef = this.modalService.open(VetDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IVet[]): IVet[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.vetService.vetsParams.set(queryObject);
  }
