...
public class OwnerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all owners").get("/api/owners").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created owner").get("${new_owner_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created owner").delete("${new_owner_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
