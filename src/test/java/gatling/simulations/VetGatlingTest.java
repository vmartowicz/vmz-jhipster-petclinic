...
public class VetGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all vets").get("/api/vets").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created vet").get("${new_vet_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created vet").delete("${new_vet_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
