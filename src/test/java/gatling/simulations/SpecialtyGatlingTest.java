...
public class SpecialtyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all specialties").get("/api/specialties").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created specialty").get("${new_specialty_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created specialty").delete("${new_specialty_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
