...
public class VisitGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all visits").get("/api/visits").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created visit").get("${new_visit_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created visit").delete("${new_visit_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
