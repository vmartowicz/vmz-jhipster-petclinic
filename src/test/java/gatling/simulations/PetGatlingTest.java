...
public class PetGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all pets").get("/api/pets").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created pet").get("${new_pet_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created pet").delete("${new_pet_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
