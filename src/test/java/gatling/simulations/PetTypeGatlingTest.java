...
public class PetTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all petTypes").get("/api/pet-types").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created petType").get("${new_petType_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created petType").delete("${new_petType_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
